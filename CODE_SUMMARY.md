...
- `Assets/Scripts/HexGame/TurnManager.cs`: Runs three round phases: simultaneous card selection, initiative reveal/sort, then execution. Chosen card initiatives sort ascending; the isolated `CompareQueueEntries()` tie-break resolves player first, then instance order. Each queued unit is revalidated immediately before its action.
...
- `Assets/Scripts/HexGame/SkillActionUI.cs`: Builds the FHD-scaled card hand and handles target-free ordered card planning. Board clicks are ignored during card selection; movement destinations, attack targets, and no-target feedback appear only during execution.
