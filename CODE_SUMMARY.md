...
- `Assets/Scripts/HexGame/SkillActionUI.cs`: Builds the existing FHD-scaled card presentation, registers cards with the dynamic hand, and handles target-free ordered card planning. Board clicks are ignored during card selection; movement destinations, attack targets, and no-target feedback appear only during execution.
- `Assets/Scripts/HexGame/SkillHandLayout.cs`: Reflows any hand size across a fixed readable area using adaptive overlap, a shallow fan arc, per-card rotation, hover neighbor separation, and live drag reordering without shrinking card dimensions.
- `Assets/Scripts/HexGame/SkillCardView.cs`: Separates card input from visual motion and spring-animates layout targets, 110% hover lift, selection, drag following, inertia tilt, focus sorting, and disabled presentation with unscaled time.
